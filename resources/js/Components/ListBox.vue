        ...
        <Listbox v-model="selectedId">
            <div class="relative">
                <ListboxButton
                    ...
                    <span class="block truncate"><span class=" capitalize">{{ props.name }}</span> : <span class="font-bold"> {{ selectedItem ? selectedItem.name : 'Select an option' }}</span></span>
                ...
                <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                    ...
                    <ListboxOptions
                        class="absolute z-[9999] mt-1 min-w-52  max-h-60 overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                        <ListboxOption v-slot="{ active, selected }" v-for="item in items" :key="item.id"
                            :value="item.id">
                            ...
                            <li :class="[
                                ...
                                <span v-if="item.icon" v-html="item.icon" :class="item.color"></span>
